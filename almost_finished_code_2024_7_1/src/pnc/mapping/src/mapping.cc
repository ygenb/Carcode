...
void OccGridMap::updateByLocalMap(const Eigen::Vector3d& sensor_p,
                           const std::vector<Eigen::Vector3d>& pc)
{
  // 1. reset map
  resetMap();

  // 2.update local range
  Eigen::Vector3i sensor_idx = pos2idx(sensor_p);
  Eigen::Vector3i offset = sensor_idx - Eigen::Vector3i(size_x / 2, size_y / 2, size_z / 2);
  if (!init_finished) {
    offset_x = offset.x();
...
// void OccGridMap::occ2pc2D(sensor_msgs::PointCloud2& msg) {
//   pcl::PointXYZ pt;
//   pcl::PointCloud<pcl::PointXYZ> pcd;
//   int z = pos2idx(Eigen::Vector3d(0.0, 0.0, 0.0))(2);   // get z=0 index
//   for (int x = 0; x < size_x; ++x) {
//     for (int y = 0; y < size_y; ++y) {
//       // Eigen::Vector3i idx(offset_x + x, offset_y + y, offset_z + z);
//       Eigen::Vector3i idx(offset_x + x, offset_y + y, z);
//       if (atId(idx) == 1) {
//         pt.x = (offset_x + x + 0.5) * resolution;
//         pt.y = (offset_y + y + 0.5) * resolution;
//         // pt.z = (offset_z + z + 0.5) * resolution;
//         pt.z = (z + 0.5) * resolution;
//         pcd.push_back(pt);
//       }
//     }
//   }
...
void OccGridMap::occ2pc2D(sensor_msgs::PointCloud2& msg) {
  ...
  {
    int z = pos2idx(Eigen::Vector3d(0.0, 0.0, h))(2);   // get z=0 index
    for (int x = 0; x < size_x; ++x) {
      for (int y = 0; y < size_y; ++y) {
        // Eigen::Vector3i idx(offset_x + x, offset_y + y, offset_z + z);
        Eigen::Vector3i idx(offset_x + x, offset_y + y, z);
