...
struct OccGridMap : public RingBuffer<int8_t> {
 ...
 public:
  ...
  inline void setup(const double& res,
    ...
    resolution = res;
    // std::cout << "resolution: " << resolution << " map_size.x(): " << map_size.x() << std::endl;
    ...
    v0.reserve(size_x * size_y * size_z);  // no use
