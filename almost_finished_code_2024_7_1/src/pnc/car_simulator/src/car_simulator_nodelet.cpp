...
struct Car {
  ...
  void step(const Eigen::Vector2d& input, const double dt) {
    // Runge–Kutta
    Eigen::Vector4d k1 = diff(state, input, dt);
    Eigen::Vector4d k2 = diff(state + k1 * dt / 2, input, dt);
    Eigen::Vector4d k3 = diff(state + k2 * dt / 2, input, dt);
    Eigen::Vector4d k4 = diff(state + k3 * dt, input, dt);
...
class Nodelet : public nodelet::Nodelet {
 private:
  ...
  struct DelayedMsg {
    ...
    double v, delta;
    DelayedMsg() {}
    DelayedMsg(const ros::Time& _t, double _v, double _delta) : t(_t), v(_v), delta(_delta) {}
  };
  ...
  void cmd_callback(const geometry_msgs::Twist::ConstPtr& msg) {
    delayedMsgs_.emplace_back(ros::Time::now(), msg->linear.x, msg->angular.z);
    // input_(0) = msg->a;
    // input_(1) = msg->delta;
  }
 ...
 public:
  void onInit(void) {
    ...
    cmd_sub_ = nh.subscribe<geometry_msgs::Twist>("/real_cmd", 1, &Nodelet::cmd_callback, this, ros::TransportHints().tcpNoDelay());
