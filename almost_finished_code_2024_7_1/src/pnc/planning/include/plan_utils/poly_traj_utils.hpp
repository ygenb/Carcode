...
namespace plan_utils
{
    ...
    public:
        ...
        inline void GetState3d(double t, Eigen::Vector3d &state) const
        {
            double inner_t = t;
        ...
        inline void GetState(double t, Eigen::VectorXd &state) const
        {
            double inner_t = t;
            if(inner_t > getTotalDuration())
                inner_t =getTotalDuration();
            ...
            // theta, curv, vel, acc, phi;
            state(2) = otherstate(0);  // heading angle
            state(3) = otherstate(1);  // curvature
            state(4) = otherstate(2);  // velocity
