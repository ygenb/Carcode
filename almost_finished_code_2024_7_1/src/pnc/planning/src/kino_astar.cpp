...
{
  KinoAstar::KinoAstar(ros::NodeHandle& nh, mapping::MapManager::Ptr mm, VehicleParam::Ptr vp) : map_(mm), vehicle_(vp)
  {
    nh.getParam("horizon", horizon_);
    nh.getParam("yaw_resolution", yaw_resolution_);
    nh.getParam("lambda_heu", lambda_heu_);
    nh.getParam("allocate_num", allocate_num_);
    nh.getParam("check_num", check_num_);
    nh.getParam("max_search_time", max_seach_time);
    nh.getParam("traj_forward_penalty", traj_forward_penalty);
    nh.getParam("traj_back_penalty", traj_back_penalty);
    nh.getParam("traj_gear_switch_penalty", traj_gear_switch_penalty);
    nh.getParam("traj_steer_penalty", traj_steer_penalty);
    nh.getParam("traj_steer_change_penalty", traj_steer_change_penalty);
    nh.getParam("step_arc", step_arc);
    nh.getParam("checkl", checkl);
    nh.getParam("max_nonsv", non_siguav);    
    ...
    nh.getParam("map_size_x", map_size_3d_(0));
    nh.getParam("map_size_y", map_size_3d_(1));
    nh.getParam("map_resl", resolution_);
    origin_(0) = -0.5*map_size_3d_(0);
    origin_(1) = -0.5*map_size_3d_(1);
    yaw_origin_ = -M_PI;

    nh.getParam("max_frontend_forward_vel", max_forward_vel);
    nh.getParam("max_frontend_forward_acc", max_forward_acc);
    nh.getParam("max_frontend_backward_vel", max_backward_vel);
    nh.getParam("max_frontend_backward_acc", max_backward_acc);
    nh.getParam("max_frontend_cur", max_cur_);
 
    max_steer_ = std::atan(vehicle_->wheel_base()*max_cur_);
    inv_resolution_ = 1.0 / resolution_;
    inv_yaw_resolution_ = 1.0 / yaw_resolution_;
    shotptr =std::make_shared<ompl::base::ReedsSheppStateSpace>(1.0/max_cur_);

    //set margain
    vehicle_->set_length(vehicle_->length()+0.1);  // margin 0.2 for 1.9
    vehicle_->set_width(vehicle_->width()+0.1);
  ...
  void KinoAstar::stateTransit(Eigen::Vector3d &state0,  Eigen::Vector3d &state1,
    ...
    if(psi!=0){
      double k = vehicle_->wheel_base()/tan(psi);
                               ...
                               Eigen::Vector4d end_state,bool use3d)
  {
    ...
    isocc = map_->CheckIfCollisionUsingPosAndYaw(start_state.head(3));
    if(isocc){
      ROS_ERROR("KinoAstar: head is not free!");
      return NO_PATH;
    }
    isocc = map_->CheckIfCollisionUsingPosAndYaw(end_state.head(3));
    if(isocc){
    ...
    while (!open_set_.empty())
    {
      ...
      double runTime = elapsed_seconds.count() * 1000 / 1000.0;  
      ...
      for (auto& input:inputs){
        ...
        for (int k = 1; k <= check_num_; ++k)
        {
          ...
          is_occ = map_->CheckIfCollisionUsingPosAndYaw(xt);
          if (is_occ)
          {
            // std::cout<<"occ!\n";
            break;
  ...
  bool KinoAstar::is_shot_sucess(Eigen::Vector3d state1,Eigen::Vector3d state2){
    ...
    for(unsigned int i = 0; i < path_list.size(); ++i){
        is_occ = map_->CheckIfCollisionUsingPosAndYaw(path_list[i]);
  ...
  double KinoAstar::evaluateLength(double curt,double locallength,double localtime,double max_vel, double max_acc, double startV, double endV){
    double critical_len; //the critical length of two-order optimal control, acc is the input
    if(startV>max_vel||endV>max_vel){
      std::cout << "startV: " << startV << " endV: " << endV << " max_vel: " << max_vel << std::endl;
  ...
  void KinoAstar::getState(Eigen::Vector4d state, Eigen::Vector2d control_input,
                                     VehicleState &v_state)
  {

    v_state.vec_position = state.head(2); 
    v_state.angle        = state(2); 
    v_state.velocity     = state(3); 

    v_state.steer        = control_input(0);
    v_state.acceleration = control_input(1);
    v_state.curvature    = std::tan(v_state.steer) / vehicle_->wheel_base();
                                  ...
                                  Eigen::MatrixXd &flat_state, int singul)
  {
    ...
    flat_state << state.head(2), init_R*Eigen::Vector2d(vel, 0.0), 
                  init_R*Eigen::Vector2d(control_input(1), std::tan(control_input(0)) / vehicle_->wheel_base() * std::pow(vel, 2));
