...
void odom_callback(const nav_msgs::Odometry::ConstPtr &msg)
{
  ...
  // @zhiwei rotate car model
  Eigen::Quaterniond q_odom(msg->pose.pose.orientation.w, msg->pose.pose.orientation.x, msg->pose.pose.orientation.y, msg->pose.pose.orientation.z);
  Eigen::AngleAxisd R1(M_PI, Eigen::Vector3d::UnitY());
  Eigen::AngleAxisd R2(M_PI/2, Eigen::Vector3d::UnitX());
  Eigen::Quaterniond q_vis = q_odom*R2*R1;
  ...
  meshROS.pose.orientation.w = q_vis.w();
  meshROS.pose.orientation.x = q_vis.x();
  meshROS.pose.orientation.y = q_vis.y();
  meshROS.pose.orientation.z = q_vis.z();
...
int main(int argc, char **argv)
{
  ...
  n.param("mesh_resource", mesh_resource, std::string("package://odom_visualization/meshes/audi.dae"));
